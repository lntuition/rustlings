...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        let red = match u8::try_from(tuple.0) {
            Ok(red) => red,
            Err(_) => return Err("Red must be 0-255".to_string()),
        };
        let green = match u8::try_from(tuple.1) {
            Ok(green) => green,
            Err(_) => return Err("Green must be 0-255".to_string()),
        };
        let blue = match u8::try_from(tuple.2) {
            Ok(blue) => blue,
            Err(_) => return Err("Blue must be 0-255".to_string()),
        };
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() != 3 {
            Err("Slice length must be 3".to_string())
        } else {
